...
import com.smartenergy.workorder.vo.SkillGroupVO;
...
public class DispatchDashboardServiceImpl implements DispatchDashboardService {
    ...
    public DispatchSummaryVO summary() {
        ...
        // ---- 工单统计 ----
        vo.setTotalOrders(existingWorkOrderMapper.selectCount(null));
        vo.setPendingOrders(existingWorkOrderMapper.selectCount(
                new QueryWrapper<ExistingWorkOrder>().eq("status", "PENDING")));
        vo.setInProgressOrders(existingWorkOrderMapper.selectCount(
                new QueryWrapper<ExistingWorkOrder>().eq("status", "IN_PROGRESS")));
        vo.setResolvedOrders(existingWorkOrderMapper.selectCount(
                new QueryWrapper<ExistingWorkOrder>().eq("status", "RESOLVED")));
