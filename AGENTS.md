...
SmartEnergyMaster（智驭能效）是一个工业能源管理平台，用于监控钢铁厂6台核心设备（电弧炉、钢包精炼炉、连铸机、循环水泵、空压机、除尘系统）的实时运行状态。系统接收传感器遥测数据，自动检测设备故障，生成维修工单，并结合分时电价给出 AI 调度建议。
...
| `data/` | Python 数据管道。`download_csv.py` 从 UCI 下载钢铁行业能耗数据集（id=851）。`data_pump.py` 是设备仿真器，模拟 6 台设备的耦合物理模型，先回填 24 小时历史数据，再以 3 秒间隔持续推送实时数据到后端 `/api/sensor/upload` 接口，并随机注入故障场景。 |
...
```
`data_pump.py` 模拟 6 台设备，通过 `POST /api/sensor/upload` 每 3 秒上报一次数据，需后端已启动。
...
- **LadleFurnaceSimulator（钢包精炼炉）**：从电弧炉状态派生——电弧炉出钢后激活精炼
- **ContinuousCasterSimulator（连铸机）**：从电弧炉状态派生——高温/高负荷时激活连铸
- **DustCollectorSimulator（除尘系统）**：从电弧炉状态派生——电弧炉运行时必须同步运行
...
| 数据仿真器 | data_pump.py（6 台设备耦合模型 + 24h 历史回填 + 3 秒实时推送 + 概率性故障注入） |
